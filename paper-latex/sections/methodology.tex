...
The Benchmarking design will be structured to evaluate the performance of all the data availability solutions under the study. 
\vspace{1em}

\subsubsection{Polkadot}
For Benchmarking of polkadot's ELVES we are using Paseo testnet and polkadots mainnet for benchmarking. The current test code implements system remarks and preimages for submission of data, measuring the transaction fees, existential deposit and time taken for finalization of the block.  

\vspace{1em}

\subsubsection{Celestia}
We are planning to use celestia's RPC endpoints and Cosmos SDKs for benchmarking.
\vspace{1em}

\subsubsection{Espresso's Tiramisu}
For Espresso's Tiramisu, we utilize their three-layered data availability architecture through their API endpoints. As of now, our testing framework implements:
\begin{itemize}
     \item Data availability metrics through REST API endpoints
...
\subsubsection{NEAR}
To test NEAR's sharded data availability, we are using NEAR's testnet and mainnet. We are planning to setup and run a local testnet node of NEAR and also use NEAR's SDKs for benchmarking.

\vspace{1em}

\subsubsection{Avail}
For Avail's benchmarking we have currently utilized avails JavaScript SDK and Python implementations. The code framework includes:
\begin{itemize}
    \item Block bloat testing with automatic payload size adjustment
    \item Telemetry probing for network metrics
    \item Data submission through AppID-based categorization
...
\hline
\textbf{Protocol} & \textbf{Polkadot} & \textbf{Celestia} & \textbf{Espresso} & \textbf{NEAR} & \textbf{Avail} \\
\hline
Throughput (MB/s) & X & X & X & X & X \\
Network Load & Low & Medium & High & Medium & Low \\
Peak Performance & X MB/s & X MB/s & X MB/s & X MB/s & X MB/s \\
\hline
\end{tabular}
\end{table}
